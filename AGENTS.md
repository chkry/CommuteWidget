...
- Verify: `./gradlew assembleDebug test lint` must be green before any commit; current suite is 745 JUnit4 unit tests, lint has 0 errors (pinned-version advisories are accepted).
...
- `engine/health/`: pure health logic (`SleepLogic` contains the strict keyguard sleep estimator with screen-event fallback only, plus WaterLogic, WalkAndSunsetLogic, NudgeLogic, HealthParams, and CustomPillLogic).
...
- `schedule/`: WorkManager workers (WindowBoundaryWorker, CalendarTickWorker, CalendarChangeWorker, CommuteLeaveByWorker, EventLeaveByWorker, HealthMorningWorker, HealthBoundaryWorker, HealthWalkNotifyWorker, EventNearWorker) and CommuteScheduler.ensureScheduled.
...
- `ui/SettingsComponents.kt`, `SettingsSummaries.kt`, `DayLabels.kt`, `RemindersHelpers.kt`, `InstalledAppPicker.kt`, and category screens: reusable settings UI, pure summaries/day labels/reminder helpers, installed-app picker helpers, and focused Compose screens for commute setup, places/maps, alerts/timing, calendar, reminders, health/experimental nudges, widget appearance, and access/app info.
...
- Calendar mode shows the next event remaining today (query extends past midnight by `eventTakeoverMinutes` so imminent after-midnight events still route).
- Located events starting within `eventTakeoverMinutes` (default 120, the unified nearness threshold) route from current location with a map, while farther located events and unlocated events render a full-width plain card at zero Google API cost.
- A calendar-event location containing a virtual-meeting URL or platform marker (`http://`/`https://`, `teams.microsoft`, `zoom.us`, `meet.google`, `webex`, or the literal 'Microsoft Teams Meeting') is normalized to unlocated at the calendar reader, before it can win the located-over-unlocated preference or enter the routed pipeline.
- A first-attempt (new-target) event-routing failure of any cause (geocode, device location, Routes, Static Maps) falls back to the plain name+time card instead of a broken routed shell (0-min ETA, warning glyph, calendar-emoji map placeholder); a same-target failure (an event that already routed successfully) is unchanged and still shows stale route/map data with the warning glyph.
- An `event_near` one-shot WorkManager wake flips a far located event to routed when it enters the nearness window.
- When no events remain today, the widget renders the wind-down card.
...
- Health layer (v6): six core features (sleep estimation from UsageStats KEYGUARD_SHOWN/KEYGUARD_HIDDEN events with screen-event fallback only for zero-keyguard-event nights and 14-day rolling history as nudge input only, evening walk advisor with window-gated pill visibility, water reminders with Health Connect hydration writes and meeting suppression, supplement routines, audiobook commute detection via MediaSession with installed-app picker, Health Connect 1.1.0 for steps/exercise) plus nine experimental toggles default OFF under **Experimental nudges** (sleep-debt brief soften, gym-day protein priority, restless-night focus shield, post-Audible walk latch, daylight walk preference, focus gap chip, post-gym water pulse, morning-light line, caffeine-cutoff line).
- Sleep tap pills: while the sleep-estimate toggle is enabled, `To bed` (bed glyph, 21:00-02:00) and `Woke up` (sunrise glyph, 04:30-10:00) record persistent timestamps and anchor the automatic keyguard estimate; the tap itself is a cheap bounded write (timestamp + snapshot strip + re-render) and the full local recompute runs on an immediately scheduled `health_boundary` run, keeping heavy reads off the roughly 10-second Glance action budget - no network, no notifications.
- Health suppression hierarchy: audiobook playback suppresses ALL health nudges; restless-night shield suppresses water and walk until first event ends or 10:00; water is suppressed only while a calendar meeting is ongoing (owner reversed the prior commute-map ban 2026-08-31 - water now competes by priority like any other pill); maps retain max two health pills, while calendar-empty and wind-down cards show up to four built-in health pills and four custom reminder pills per row with `+N` overflow; 2x2 renders zero health UI; health elements never inherit pending alpha or stale grey.
- Custom pill reminders (v7): up to six user-defined, widget-only reminders with up to four selected weekday slots each (configured with two-row Mo-Th/Fr-Su day chips to prevent overflow); their shared active window defaults to 60 minutes and is configurable from 15-240 minutes; an active occurrence carries over at 60 percent alpha until dismissed, midnight, or the next non-dismissed slot for that pill; taps dismiss only that `pillId:slotMinute` occurrence locally with no notification or network work; a "Reset dismissed reminders" action in the Reminders screen clears today's `customPillTakenSlots` only, preserving all other day state.
...
- "Recalculate last night's sleep" (Alerts & timing > Today) clears only the current morning's stored sleep fields (steps preserved), un-dismisses the sleep pill, and recomputes - the unfreeze path for a value written by an older estimator.
...
- 2026-08-31 usability fixes, health window gating, water overhaul, and calendar nearness routing: owner-approved updates across five areas.
Installed-app picker for audiobooks: replaced manual package-name typing in Health settings with a searchable installed launchable apps picker dialog (`InstalledAppPicker`), backed by a narrow launcher-intent `<queries>` visibility block in the manifest (no `QUERY_ALL_PACKAGES`); storage remains `Set<String>` (default `com.audible.application`); uninstalled selected packages remain visible and removable in the picker.
Reminder day picker layout: split custom pill reminder editor day chips into two rows (Mo-Th and Fr-Su) to eliminate horizontal overflow that clipped Sa/Su on standard dialog widths; pure layout fix with no data changes.
Walk pill window gating: evening walk pill previously appeared prematurely throughout the day whenever steps lagged; `suggestWalk` now returns no suggestion before search window start (default 18:00), the widget filter drops `WALK` nudges before start minute as stale-snapshot defense, the `health_boundary` chain wakes at window start so the pill appears unattended, the once-per-day walk notification cannot be scheduled before window start, and the sedentary-afternoon trigger now first surfaces at window start.
Water reminders overhaul and commute-map reversal: owner reversed the water-on-commute-map ban - water now competes purely by priority and the max-2 map cap, suppressed only while a calendar meeting is ongoing (`waterCandidate` computation-time gate).
The water anchor window (`waterFirstAnchorMinuteOfDay`/`waterLastAnchorMinuteOfDay`, default 07:30-19:30) is owner-configurable via "Window start"/"Window end" settings rows; drop cutoff is derived as window end + 30 minutes; inverted windows yield an empty plan without crashing.
Editing the window or count replans remaining slots immediately (`replanTodayWaterSlots`) preserving `waterTapMinutes` and reschedules the boundary chain with `REPLACE`.
The boundary chain wakes at meeting start/end edges that fall inside an active water slot window (local calendar read, zero new Google API calls) so pills restore promptly when meetings end.
Far located events render plain: in calendar mode, located events route (geocode + Routes + Static Maps) only when starting within `eventTakeoverMinutes` (default 120, the unified nearness threshold for both commute takeover and calendar routing); farther out they render as plain name+time cards at zero Google API cost, saving substantial API quota; a one-shot `event_near` worker (`REPLACE` policy, cancelled on calendar refresh) wakes at start minus threshold to route the event; taps on far events stay plain; the 20-minute tick arms only on routed events; accepted known edge: leave-by notifications are computed when routing occurs, so events requiring more lead time than threshold notify late.
- 2026-08-31 junk calendar locations and event-routing failure fallback: office-email invites often carry a virtual-meeting URL or platform name (Microsoft Teams links, `zoom.us`, `meet.google`, Webex, or the literal 'Microsoft Teams Meeting') as the event location, which used to enter the routed pipeline, fail geocoding or routing, and render as a broken shell (0-min ETA, warning glyph, calendar-emoji placeholder) while also spending a geocode/Routes call every refresh; `isRoutableEventLocation` in `CalendarReader.kt` now normalizes those markers to no location at the single point `selectTodayEvent` reads location from (`RawInstance.routableLocation`), so a junk-located event is treated as unlocated everywhere downstream (event takeover, calendar-mode routing, the far-event gate, leave-by, the event_near flip) and never spends a call; separately, `failureSnapshot` now falls back to the plain name+time card (`calendarPlainEventSnapshot`'s shape, `lastFetchFailed = false`) for a first-attempt (new-target) `CALENDAR_EVENT` routing failure of any cause, health fields carried forward exactly as every other branch there does, while a same-target failure (an event that already routed successfully once) is unchanged and still shows stale route/map data with the warning glyph.
- 2026-08-31 room-capacity-suffix locations and same-target event-failure convergence: an Outlook conference-room resource such as "PA HQ-1-FOREST (4)" passed the junk-marker filter (no URL, no platform name) but never geocoded (ZERO_RESULTS every attempt), and because `failureSnapshot`'s same-target branch preserved ANY previous `CALENDAR_EVENT` snapshot unconditionally, a broken shell already stored for that event (duration 0, no map) got re-preserved forever instead of ever reaching the plain-card fallback.
`isRoutableEventLocation` now also rejects a trimmed location ending in a bare-integer parenthesized suffix (`\(\d+\)$`, e.g. the room's seat count), so a fresh Outlook room resource is filtered before it ever reaches routing; "Plot 12 (Phase 2)" and any address without a trailing bare-integer suffix are unaffected.
Independently, `failureSnapshot`'s same-target `CALENDAR_EVENT` preservation now additionally requires the previous snapshot to be usable (`durationSeconds > 0` or a non-null `mapImagePath`) before treating it as last-known-good; an unusable (never-routed) previous now falls through to the same plain-card fallback the new-target branch already used, so an already-broken shell converges to the plain card on the very next failed retry instead of persisting indefinitely.
COMMUTE-mode same-target preservation is unconditional as before.
- 2026-08-31 sleep-estimation redesign: the prior longest-screen-off heuristic was replaced because notification screen-ons split a night and because an in-progress computation closed an open segment at 13:00, producing inflated frozen values such as `Slept ~12h`; the strict primary model uses real UsageStats keyguard lock/unlock pairs with no merge, tap anchors that narrow rather than replace the search, a screen-event fallback only for zero-keyguard-event nights, and live recomputation until a 10:00 or Woke Up freeze; owner-approved `To bed` and `Woke up` pills persist anchors and locally recompute, card rows expand to four built-in and four custom pills while maps stay unchanged, and on-device LLM processing and Health Connect sleep read/write were declined because ML Kit GenAI Prompt API is unsupported on the S24 Ultra, deterministic interval math gains nothing from an LLM, and no new permissions were wanted.
...
- Custom pill occurrences are fully resolved at computation time but stored UNCAPPED; the render layer re-filters against the live taken set for dismissal (never for suppression) and only then applies the surface-specific cap (four on calendar-empty cards, three on maps) and derives the `+N` overflow - capping before the dismissal filter strands stale or lone overflow indicators after taps.
...
- A sleep estimate defers until an end exists and freezes only at 10:00 or on a valid Woke Up tap; the once-per-day write guard is gone, and recomputation remains live until that freeze.
- To Bed and Woke Up taps narrow the strict keyguard search domain and never replace keyguard pairing; only anchored nights may bypass the three-hour plausibility floor.
- The legacy screen-event estimator is fallback-only for a night with zero keyguard events; keyguard data with no qualifying interval is an honest no-estimate result.
...
- Event leave-by notifications are computed when routing occurs within `eventTakeoverMinutes`, so an event needing more lead time than the takeover threshold notifies late.
- The junk-location marker list is conservative: a location that is neither a URL, a known marker, nor a trailing bare-integer parenthesized suffix (a bare conference-room code with no capacity number, say) still attempts routing once and lands on the plain card via the event failure fallback after spending the geocode/Routes calls.
- A same-target event failure whose previous snapshot never actually routed (duration 0, no map) now falls back to the plain card on the very next retry instead of preserving the broken shell, so a never-geocodable event (an Outlook room resource, an address Google cannot find) converges within one failed refresh rather than staying broken forever.

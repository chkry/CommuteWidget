...
- Verify: `./gradlew assembleDebug test lint` must be green before any commit; current suite is 738 JUnit4 unit tests, lint has 0 errors (pinned-version advisories are accepted).
...
- The junk-location marker list is conservative: a location that is neither a URL nor a known marker (a bare conference-room code, say) still attempts routing once and lands on the plain card via the event failure fallback after spending the geocode/Routes calls.

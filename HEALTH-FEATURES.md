...
Anti-nag rules cap map density at two built-in health pills and card density at four built-in pills plus four custom reminder pills per row, suppress all health UI during audiobook playback, never backfill missed water slots, and keep health elements out of pending or stale route styling.
Water competes for the map pill slots by priority like any other pill (owner-approved 2026-08-31); it is suppressed only while a calendar meeting is ongoing.
...
| Water reminders | Water reminders | ON | Up to five dynamic hydration slots per day across a configurable window (07:30-19:30 default); tap logs 250 ml to Health Connect. |
...
**What it does:** When steps are behind goal or the afternoon is sedentary, computes a walk duration and searches the configured window (default 18:00-21:30) for a calendar gap, avoiding events and the To Home commute window, finishing at least 60 minutes before typical bedtime.
The pill is window-gated and appears only once the walk window opens; `suggestWalk` returns no suggestion before search window start, and the widget filter drops walk nudges before the start minute.
The `health_boundary` chain wakes at window start so the pill appears unattended, and the sedentary-afternoon trigger surfaces at window start.
Shows pill `Walk 7:30` and fires one mutex-guarded notification at walk start (cannot be scheduled before window start).

| Parameter | Value | Evidence |
| --- | --- | --- |
| Step goal | 8000 (settings-configurable) | convention |
| Sedentary trigger | Under 1500 steps since noon, checked from 16:00 (surfaces at walk window start) | convention |
...
| Pill visibility | Window-gated: visible only after search window start; boundary chain wakes at window start | convention |
| Bedtime buffer | Finish 60+ minutes before typical bedtime (derived from 14-day sleep history) | moderate |
| Notification | One per day at slot start via `health_walk_notify`, scheduled at or after window start; deduped by `walkNotified` | convention |

## Water reminders

**Toggle:** Water reminders (default ON).

**What it does:** Plans dynamic hydration slots anchored across a configurable window (07:30-19:30 default, "Window start"/"Window end" in settings), shifted around calendar meetings, with 90-minute minimum spacing.
Shows one active `Drink water` pill for a 30-minute window, suppressed while a calendar meeting is ongoing.
Tap logs 250 ml to Health Connect and dismisses; write failure keeps the pill for retry.
Editing the window or the reminders-per-day count replans today's remaining slots immediately (owner-approved 2026-08-31) rather than waiting for the next day's rollover; already-tapped slots are preserved.

| Parameter | Value | Evidence |
| --- | --- | --- |
| Reminders per day | 5 default, configurable 3-8 | convention |
| Anchor range | 07:30-19:30 default, configurable via "Window start"/"Window end" | convention |
| Minimum spacing | 90 minutes between shown or dismissed slots | convention |
| Active window | 30 minutes per slot | convention |
| Cutoff | Window end + 30 minutes (20:00 at default anchors) | weak-moderate |
...
| Meeting suppression | No candidate emitted while a meeting is ongoing (start inclusive, end exclusive); the boundary chain also wakes at in-window meeting edges so the pill reappears promptly when a meeting ends | owner-approved 2026-08-31 |
| Commute map | Competes by priority like any other pill, capped at two visible map pills; no longer a hard ban | owner-approved 2026-08-31 (reverses prior anti-nag convention) |
...
The package list is selectable from installed launchable apps via a searchable picker dialog in settings (no manual package name entry required).
...
| Package selection | Searchable installed-app picker dialog in Health settings | convention |
...
- **Water on commute map:** Competes by priority like any other pill, capped at two; suppressed only while a calendar meeting is ongoing (owner-approved 2026-08-31, reverses the prior hard ban).
...
| `HealthBoundaryWorker` | `health_boundary` | One-shot chain to next transition | Recomputes health fields only (no network); wakes at water and custom-reminder slot starts and active-window ends, today's meeting start/end edges that land inside an active water slot window (a local calendar provider read, not a new API call), supplement window edges, 21:30 vitamins cutoff, 18:00 protein transition, walk target, shield end, caffeine lead/cutoff, To Bed and Woke Up pill boundaries at 21:00, 02:00, 04:30, and 10:00 when untapped, and midnight rollover |
